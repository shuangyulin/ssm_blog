...
function initCommentManageTool() {
	comment_manage_tool = {
		init: function() {
			$.ajax({
				...
				success : function (data, response, status) {
					$("#blogObj_blogId_query").combobox({ 
					    valueField:"blogId",
					    textField:"title",
					    panelHeight: "200px",
				        editable: false, //不允许手动输入 
					});
					data.splice(0,0,{blogId:0,title:"不限制"});
					$("#blogObj_blogId_query").combobox("loadData",data); 
				}
			});
			$.ajax({
		...
		search: function() {
			...
			queryParams["blogObj.blogId"] = $("#blogObj_blogId_query").combobox("getValue");
				...
				$.ajax({
					...
					success : function (comment, response, status) {
						...
						if (comment) { 
							...
							$("#comment_blogObj_blogId_edit").combobox({
								url:"Blog/listAll",
							    valueField:"blogId",
							    textField:"title",
							    panelHeight: "auto",
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#comment_blogObj_blogId_edit").combobox("select", comment.blogObjPri);
									//var data = $("#comment_blogObj_blogId_edit").combobox("getData"); 
						            //if (data.length > 0) {
						                //$("#comment_blogObj_blogId_edit").combobox("select", data[0].blogId);
						            //}
								}
							});
							...
							$("#comment_commentTime_edit").datetimebox({
