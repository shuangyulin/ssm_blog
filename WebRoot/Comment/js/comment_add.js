$(function () {
	$("#comment_blogObj_blogId").combobox({
	    url:'Blog/listAll',
	    valueField: "blogId",
	    textField: "title",
	    panelHeight: "auto",
        ...
        onLoadSuccess: function () { //数据加载完毕事件
            var data = $("#comment_blogObj_blogId").combobox("getData"); 
            if (data.length > 0) {
                $("#comment_blogObj_blogId").combobox("select", data[0].blogId);
            }
        }
	});
	...
	$("#comment_commentTime").datetimebox({
	    required : true, 
	    showSeconds: true,
	    editable: false
	});
